...
const (
	...
	RedisCmdClusterSlots    = "Cluster Slots"    //获取集群节点的映射数组
	RedisCmdConfigSet       = "Config Set"       //修改 RedisCmd配置参数，无需重启
	RedisCmdCommandInfo     = "Command Info"     //获取指定 RedisCmd命令描述的数组
	RedisCmdShutdown        = "Shutdown"         //异步保存数据到硬盘，并关闭服务器
	RedisCmdSync            = "Sync"             //用于复制功能(replication)的内部命令
	RedisCmdClientKill      = "Client Kill"      //关闭客户端连接
	RedisCmdRole            = "Role"             //返回主从实例所属的角色
	RedisCmdMonitor         = "Monitor"          //实时打印出 RedisCmd服务器接收到的命令，调试用
	RedisCmdCommandGetkeys  = "Command Getkeys"  //获取给定命令的所有键
	RedisCmdClientGetname   = "Client Getname"   //获取连接的名称
	RedisCmdConfigResetstat = "Config Resetstat" //重置 INFO 命令中的某些统计数据
	RedisCmdCommandCount    = "Command Count"    //获取 RedisCmd命令总数
	RedisCmdTime            = "Time"             //返回当前服务器时间
	RedisCmdInfo            = "Info"             //获取 RedisCmd服务器的各种信息和统计数值
	RedisCmdConfigRewrite   = "Config rewrite"   //对启动 RedisCmd服务器时所指定的 redis.conf 配置文件进行改写
	RedisCmdClientList      = "Client List"      //获取连接到服务器的客户端连接列表
	RedisCmdClientSetname   = "Client Setname"   //设置当前连接的名称
	RedisCmdBgsave          = "Bgsave"           //在后台异步保存当前数据库的数据到磁盘

	//脚本 命令
	RedisCmdScriptkill   = "Script kill"   //杀死当前正在运行的 Lua 脚本。
	RedisCmdScriptLoad   = "Script Load"   //将脚本 script 添加到脚本缓存中，但并不立即执行这个脚本。
	RedisCmdEval         = "Eval"          //执行 Lua 脚本。
	RedisCmdEvalsha      = "Evalsha"       //执行 Lua 脚本。
	RedisCmdScriptExists = "Script Exists" //查看指定的脚本是否已经被保存在缓存当中。
	RedisCmdScriptFlush  = "Script Flush"  //从脚本缓存中移除所有脚本。
